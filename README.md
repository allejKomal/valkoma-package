...
`valkoma-package` is a modular, type-safe React component library built with TypeScript and Vite, featuring a comprehensive set of UI components from shadcn/ui. The library is designed for easy integration into React projects, with components organized into two main modules: `primitive` (containing all shadcn/ui components) and `design-system` (reserved for custom components). It leverages Tailwind CSS for styling, ensuring customizable and responsive UI elements.

## Purpose

This library provides a collection of reusable, accessible, and customizable UI components for building modern web applications. It includes all shadcn/ui components (e.g., Button, Card, Accordion) under the `primitive` module, making it ideal for developers who want a robust set of UI building blocks with TypeScript support and Tailwind CSS styling.

## Features

- **Comprehensive shadcn/ui Components**: Includes all shadcn/ui components such as Accordion, Button, Card, Dialog, and more, accessible via `valkoma-package/primitive`.
- **Modular Imports**: Supports structured imports for `primitive` (shadcn/ui components) and `design-system` (future custom components).
